...
<template>
  <div
    class="min-h-screen flex flex-col md:flex-row bg-[var(--color-background)]"
  >
    <!-- Branding Section -->
    <div
      class="md:w-1/3 lg:w-2/5 bg-[var(--bg-deep-wave)] flex flex-col items-center justify-center p-8 text-primary relative overflow-hidden"
    >
      <!-- Decorative background elements -->
      <div
        class="absolute top-[-10%] left-[-10%] w-64 h-64 bg-white opacity-5 rounded-full blur-3xl"
      ></div>
      <div
        class="absolute bottom-[-10%] right-[-10%] w-96 h-96 bg-secondary opacity-10 rounded-full blur-3xl"
      ></div>

      <div class="relative z-10 flex flex-col items-center text-center">
        <img
          :src="logo"
          alt="Surf Accounting Logo"
          class="h-24 w-24 md:h-32 md:w-32 mb-8"
        />
        <h1
          class="text-4xl md:text-5xl lg:text-6xl font-black tracking-tight mb-2 drop-shadow-md"
        >
          Surf Accounting
        </h1>
      ...
      <div class="max-w-md w-full">
        ...
        <div v-if="view === 'login'" class="space-y-8 animate-fade-in">
          ...
          <form class="space-y-6" @submit.prevent="handleLogin">
            ...
            <button
              type="submit"
              class="btn-primary w-full py-3 text-lg font-bold shadow-lg"
            >
              Sign in
            </button>
            ...
            <p
              v-if="message"
              class="text-center text-sm text-success font-bold"
            >
              {{ message }}
            </p>
          </form>
        ...
        <div v-if="view === 'forgot'" class="space-y-8 animate-fade-in">
          <div>
            <button
              @click="view = 'login'"
              class="flex items-center text-primary font-bold mb-4 hover:translate-x-[-4px] transition-transform"
            >
