<template>
  <div
    v-if="invoice"
    class="p-10 max-w-4xl mx-auto bg-white"
    id="printable-area"
  >
    <div class="flex justify-between items-start border-b pb-8 mb-8">
      ...
      <div class="text-right">
        ...
        <p class="text-gray-600">
          {{ settings?.address || "123 Business Rd" }}
        </p>
        <p class="text-gray-600">
          {{ settings?.city || "City" }}, {{ settings?.state || "ST" }}
          {{ settings?.zip || "12345" }}
        </p>
        <p class="text-gray-600">
    ...
    <table class="w-full mb-12">
      <thead>
        <tr class="border-b-2 border-gray-300">
          <th class="py-3 text-left font-bold text-gray-700 uppercase text-sm">
            Description
          </th>
          <th
            class="py-3 text-right font-bold text-gray-700 uppercase text-sm w-24"
          >
            Qty
          </th>
          <th
            class="py-3 text-right font-bold text-gray-700 uppercase text-sm w-32"
          >
            Price
          </th>
          <th
      ...
      <tbody class="divide-y">
        <tr v-for="line in invoice.lines" :key="line.id">
          ...
          <td class="py-4 text-right text-gray-800 font-medium">
      ...
      <div class="w-64 space-y-3">
        <div class="flex justify-between text-sm">
          ...
          <span class="text-gray-800 font-medium"
            >${{ Number(invoice.subtotal).toFixed(2) }}</span
          >
        </div>
        <template
          v-if="invoice.tax_breakdown && invoice.tax_breakdown.length > 1"
        >
          <div
            v-for="tax in invoice.tax_breakdown"
            :key="tax.id"
            class="flex justify-between text-sm"
          >
            <span class="text-gray-600 font-bold uppercase"
              >{{ tax.name }}:</span
            >
            <span class="text-gray-800 font-medium"
              >${{ Number(tax.amount).toFixed(2) }}</span
            >
          </div>
        </template>
        <div v-else class="flex justify-between text-sm">
          ...
          <span class="text-gray-800 font-medium"
            >${{ Number(invoice.tax).toFixed(2) }}</span
          >
        </div>
        <div
          class="flex justify-between text-xl border-t-2 border-gray-800 pt-3"
        >
          <span class="font-bold text-gray-800">TOTAL:</span>
          <span class="font-bold text-gray-800"
            >${{ Number(invoice.total).toFixed(2) }}</span
          >
        </div>
        <div
          v-if="invoice.balance < invoice.total"
          class="flex justify-between text-sm text-green-600 font-bold"
        >
          <span>Amount Paid:</span>
          <span
            >-${{
              (Number(invoice.total) - Number(invoice.balance)).toFixed(2)
            }}</span
          >
        </div>
        <div
          class="flex justify-between text-lg font-bold bg-gray-100 p-2 rounded mt-2"
        >
          <span class="text-gray-800">BALANCE DUE:</span>
          <span class="text-gray-900"
            >${{ Number(invoice.balance).toFixed(2) }}</span
          >
        </div>
      </div>
    </div>
