    ...
    <div id="report-content" class="card p-0 overflow-hidden min-h-[600px]">
      ...
      <div v-else-if="reportData" class="max-w-3xl mx-auto space-y-12 mb-12">
        ...
        <section>
          ...
          <div class="space-y-3 px-4 text-on-surface">
            ...
            <div class="flex justify-between">
              ...
              <span class="font-medium">{{
                formatCurrency(reportData.change_in_ar)
              }}</span>
            </div>
            <div
              v-for="item in reportData.ar_breakdown"
              :key="item.id"
              class="flex justify-between pl-6 text-sm italic text-muted"
            >
              <span>{{ item.name }}</span>
            ...
            <div class="flex justify-between">
              ...
              <span class="font-medium">{{
                formatCurrency(reportData.change_in_ap)
              }}</span>
            </div>
            <div
              v-for="item in reportData.ap_breakdown"
              :key="item.id"
              class="flex justify-between pl-6 text-sm italic text-muted"
            >
              <span>{{ item.name }}</span>
            ...
            <div
              v-if="reportData.change_in_amort !== 0"
              class="flex justify-between"
            >
              <span>Adjustments: Amortization</span>
              <span class="font-medium">{{
                formatCurrency(reportData.change_in_amort)
              }}</span>
            </div>
            <div
              class="flex justify-between pt-3 border-t border-divider font-bold mt-4"
            >
        ...
        <section v-if="reportData.net_investing_cash !== 0">
          ...
          <div class="space-y-3 px-4 text-on-surface">
            <div class="flex justify-between">
              ...
              <span class="font-medium">{{
                formatCurrency(reportData.net_investing_cash)
              }}</span>
            </div>
          </div>
        </section>
        ...
        <section v-if="reportData.net_financing_cash !== 0">
          ...
          <div class="space-y-3 px-4 text-on-surface">
            <div
              v-for="item in reportData.financing_breakdown"
              :key="item.id"
              class="flex justify-between pl-6 text-sm italic text-muted"
            >
              <span>{{ item.name }}</span>
            ...
            <div
              class="flex justify-between pt-3 border-t border-divider font-bold"
            >
              <span>Net cash from Financing Activities</span>
              <span class="font-medium">{{
                formatCurrency(reportData.net_financing_cash)
              }}</span>
            </div>
          </div>
        </section>
        ...
        <div class="pt-6 space-y-4 px-4">
          <div
            class="flex justify-between border-t border-divider pt-4 font-bold text-on-surface"
          >
...
const exportPDF = () => {
  reportService.exportToPDF(
    "report-content",
    `Cash-Flow-${filters.startDate}-to-${filters.endDate}.pdf`,
  );
};
