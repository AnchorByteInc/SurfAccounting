    ...
    <div v-else-if="reportData" class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="card card-hover">
        <div class="flex items-center justify-between mb-6">
          ...
          <span
            :class="
              reportData.trial_balance.status === 'pass'
        ...
        <p class="text-sm text-muted mb-6">
          Checks if the sum of all debits and credits in the general ledger is
          zero.
        </p>
        <div
          class="flex justify-between items-center pt-4 border-t border-divider"
        >
          <span
            class="text-sm font-bold text-muted uppercase tracking-widest text-[10px]"
            >Net Balance</span
          >
          <span
            :class="
      ...
      <div class="card card-hover">
        <div class="flex items-center justify-between mb-6">
          ...
          <span
            :class="
              reportData.balance_sheet.status === 'pass'
                ? 'bg-green-100 text-green-800'
                : 'badge-primary'
            "
            class="badge"
        ...
        <div class="space-y-3 pt-4 border-t border-divider">
          <div class="flex justify-between text-sm">
            ...
            <span class="text-on-surface font-medium">{{
              formatCurrency(reportData.balance_sheet.assets)
            }}</span>
          </div>
          <div class="flex justify-between text-sm">
            ...
            <span class="text-on-surface font-medium">{{
              formatCurrency(
                reportData.balance_sheet.liabilities +
                  reportData.balance_sheet.equity,
              )
            }}</span>
          </div>
          <div
            class="flex justify-between items-center pt-3 border-t border-divider font-bold"
          >
            <span
              class="text-sm text-muted uppercase tracking-widest text-[10px]"
              >Difference</span
            >
            <span
              :class="
      ...
      <div class="card card-hover">
        <div class="flex items-center justify-between mb-6">
          ...
          <span
            :class="
              reportData.ar_subsidiary.status === 'pass'
                ? 'bg-green-100 text-green-800'
                : 'badge-primary'
            "
            class="badge"
        ...
        <p class="text-sm text-muted mb-6">
          Ensures the sum of all customer balances matches the Accounts
        ...
        <div class="space-y-3 pt-4 border-t border-divider text-sm">
          <div class="flex justify-between">
            ...
            <span class="text-on-surface font-medium">{{
              formatCurrency(reportData.ar_subsidiary.customer_total)
            }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-muted">GL Account Balance</span>
            <span class="text-on-surface font-medium">{{
              formatCurrency(reportData.ar_subsidiary.gl_balance)
            }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="card card-hover">
        <div class="flex items-center justify-between mb-6">
          ...
          <span
            :class="
              reportData.ap_subsidiary.status === 'pass'
                ? 'bg-green-100 text-green-800'
                : 'badge-primary'
            "
            class="badge"
        ...
        <p class="text-sm text-muted mb-6">
          Ensures the sum of all vendor balances matches the Accounts Payable GL
          account.
        </p>
        <div class="space-y-3 pt-4 border-t border-divider text-sm">
          <div class="flex justify-between">
            ...
            <span class="text-on-surface font-medium">{{
              formatCurrency(reportData.ap_subsidiary.vendor_total)
            }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-muted">GL Account Balance (Credit)</span>
            <span class="text-on-surface font-medium">{{
              formatCurrency(-reportData.ap_subsidiary.gl_balance)
            }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="card card-hover md:col-span-2 p-0 overflow-hidden">
        <div class="p-6">
          <div class="flex items-center justify-between mb-6">
            ...
            <span
              :class="
                reportData.journal_entries.status === 'pass'
                  ? 'bg-green-100 text-green-800'
                  : 'badge-primary'
              "
              class="badge"
          ...
          <p class="text-sm text-muted">
            Checks if any journal entries in the system are currently
            unbalanced.
          </p>
        </div>

        <div
          v-if="reportData.journal_entries.unbalanced_count > 0"
          class="border-t border-divider"
        >
          <h4 class="text-sm font-bold text-error p-6 mb-0">
            Unbalanced Entries Found:
            {{ reportData.journal_entries.unbalanced_count }}
          </h4>
          <div class="overflow-x-auto">
            <table class="standard-table">
              ...
              <tbody>
                <tr
                  v-for="je in reportData.journal_entries.unbalanced_details"
                  :key="je.id"
                >
                  <td class="whitespace-nowrap text-muted">{{ je.id }}</td>
                  <td class="whitespace-nowrap text-muted">{{ je.date }}</td>
                  <td class="whitespace-nowrap text-on-surface font-medium">
                    {{ je.reference }}
                  </td>
        ...
        <div
          v-else
          class="text-center py-8 text-primary font-bold border-t border-divider"
        >
