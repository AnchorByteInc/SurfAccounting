<template>
  <div class="space-y-6">
    ...
    <div class="card">
      ...
      <form @submit.prevent="handleCreate" class="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
        <div class="mb-0">
      ...
      <table class="standard-table">
        ...
        <tbody>
          <tr v-for="period in periods" :key="period.id">
            <td class="whitespace-nowrap font-medium text-on-surface">{{ period.name }}</td>
            <td class="whitespace-nowrap text-muted">{{ formatDate(period.start_date) }}</td>
            <td class="whitespace-nowrap text-muted">{{ formatDate(period.end_date) }}</td>
            <td class="whitespace-nowrap">
              <span 
                class="badge"
                :class="period.is_closed ? 'btn-error' : 'bg-green-100 text-green-800'"
              >
...
const handleCreate = async () => {
  ...
  try {
    ...
    alert('Error creating period: ' + (error.response?.data?.message || error.message));
  } finally {
    loading.value = false;
  }
};
...
const handleClose = async (id) => {
  if (!confirm('Are you sure you want to close this accounting period? This will lock all transactions within its date range.')) {
    return;
  }
  try {
    ...
    const message = error.response?.data?.message || error.response?.data?.msg || 'Failed to close period.';
    alert(message);
  }
};
...
const handleDelete = async (id) => {
  ...
  try {
    ...
    const message = error.response?.data?.message || error.response?.data?.msg || 'Failed to delete period.';
    alert(message);
  }
};
