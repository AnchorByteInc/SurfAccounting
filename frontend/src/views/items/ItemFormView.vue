<template>
  <div class="max-w-3xl mx-auto space-y-6">
    <form @submit.prevent="handleSubmit" class="card space-y-6">
      ...
      <div
        class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t border-divider"
      >
        <!-- Sellable Section -->
        <div class="space-y-4">
            ...
            <div class="form-group">
              ...
              <div class="grid grid-cols-1 gap-2">
                ...
                >
                  ...
                  />
                  <span class="text-sm"
                    >{{ tax.name }} ({{ (tax.rate * 100).toFixed(1) }}%)</span
                  >
                </label>
              </div>
              <p v-if="taxes.length === 0" class="text-xs text-muted">
                No taxes defined.
              </p>
            </div>
          </div>
...
const expenseAccounts = computed(() =>
  accounts.value.filter((a) =>
    ["expense", "expenses", "cost of goods sold"].includes(
      a.type.toLowerCase(),
    ),
  ),
);
  ...
  try {
    ...
    if (isEdit.value) {
      ...
      form.value = {
        ...
        sales_tax_ids: data.sales_taxes
          ? data.sales_taxes.map((t) => t.id)
          : [],
      };
    }
  } catch (error) {
